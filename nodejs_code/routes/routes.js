...
class Routes{
	constructor(app, port_routes, ServidorDaEquipa, array_children_pids, array_children_ports){
	...
	app.get("/account", function(req, res) {
    	var accountMock = {"username": "nraboy", "password": "1234", "twitter": "@nraboy"}
		var accountTest = {"username": "test", "password": "test", "twitter": "@test"}
	...
	app.get("/start_server", function(req, res) {	
		start_server(port_routes, array_children_pids, array_children_ports);
		port_routes++;
	});

	//http://localhost:3000/localhost:3000/stop_server
	app.get("/stop_server", function(req, res) {
		stop_server(port_routes, array_children_pids, array_children_ports);
	});
...
function start_server(port_routes, array_children_pids, array_children_ports){
	var spawn = require('child_process').spawn;
	var child = spawn('node', ['routes/SerdidorDaEquipa.js', port_routes]);
	...
	console.log(array_children_pids);
	console.log(array_children_ports);
...
function stop_server(port_routes, array_children_pids, array_children_ports){
